...
for episode in range(epochs):
    ...
    obs = env.reset()
    state = to_state(obs)
    E = np.zeros(shape = (len(pos) + 1, len(vel) + 1, len(ang_pos) + 1, len(ang_vel) + 1, env.action_space.n))  # initializing eligibility traces for all state action pairs
...
df['epsilon'] = epsilon
path = 'Experiments'
df.to_csv(path+'/SARSA_lambda_1.csv')
...
df1['gamma'] = gamma
df1['epsilon'] = epsilon
df1.to_csv(path+'/SARSA_lambda_1_all.csv')
