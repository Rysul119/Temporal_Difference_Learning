...
class AC_NN(tf.keras.Model):
    ...
    def __call__(self, state, training = False):
        ...
        actor_dist = tf.nn.softmax(self.actor_out(actor_dist), axis = 1)
...
def adv_ac(env):
    ...
    for episode in range(episodes):
        ...
        for step in range(steps):
            value, prob_dist = a2c_nn(state, training=True)
            ...
            if done or step == (steps - 1):
                qval, _ = a2c_nn.__call__(new_state, training= True)
        ...
        qvals[-1] = rewards[-1] #assigning end qval equal to reward
        for t in range(len(rewards)-1):
            qvals[t] = rewards[t] + gamma * values[t+1]
        ...
        with tf.GradientTape() as tape:
            tape.watch(a2c_nn.trainable_variables)
